...
export default function WorkOrdersPage() {
  ...
  const handleProcessPayroll = async () => {
    ...
    const selectedOrdersData = workOrders.filter(order => selectedOrdersArray.includes(order.id))
    
    // Crear nuevo período de nómina
    const newPayrollPeriod = {
      id: `payroll-${Date.now()}`,
      name: `Período ${new Date().toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}`,
      startDate: new Date().toISOString().split('T')[0],
      endDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // +15 días
      status: 'draft' as const,
      totalEmployees: selectedOrdersData.reduce((sum, order) => sum + order.employeeCount, 0),
      totalGrossPay: selectedOrdersData.reduce((sum, order) => sum + order.totalSalary, 0),
      totalDeductions: selectedOrdersData.reduce((sum, order) => sum + (order.totalSalary * 0.2), 0), // 20% deducciones estimadas
      totalNetPay: selectedOrdersData.reduce((sum, order) => sum + (order.totalSalary * 0.8), 0), // 80% neto estimado
      createdDate: new Date().toISOString().split('T')[0],
      workOrders: selectedOrdersData.map(order => ({
        id: order.id,
        name: order.name,
        employerName: order.employerName,
        organizationName: order.organizationName,
        employeeCount: order.employeeCount,
        grossPay: order.totalSalary,
        deductions: {
          tax: order.totalSalary * 0.15, // 15% impuestos
          socialSecurity: order.totalSalary * 0.04, // 4% seguridad social
          other: order.totalSalary * 0.01 // 1% otros
        },
        netPay: order.totalSalary * 0.8, // 80% neto
        status: 'pending' as const,
        employees: [] // Se llenarían con datos reales
      }))
    }
    ...
    // Guardar el nuevo período en localStorage para que lo vea la página de payroll
    const existingPayrolls = JSON.parse(localStorage.getItem('payrollPeriods') || '[]')
    existingPayrolls.unshift(newPayrollPeriod) // Agregar al inicio
    localStorage.setItem('payrollPeriods', JSON.stringify(existingPayrolls))
    
    // Redirigir a la página de nómina
    router.push('/payroll')
  }
