...
export default function PayrollPage() {
  ...
  // Cargar períodos desde localStorage cuando el componente se monta
  useEffect(() => {
    const savedPayrolls = localStorage.getItem('payrollPeriods')
    if (savedPayrolls) {
      const parsedPayrolls = JSON.parse(savedPayrolls)
      if (parsedPayrolls.length > 0) {
        // Combinar períodos guardados con los mock (los guardados primero)
        const allPeriods = [...parsedPayrolls, ...mockPayrollPeriods]
        setPayrollPeriods(allPeriods)
        setSelectedPeriod(allPeriods[0]) // Seleccionar el más reciente
      }
    }
  }, [])
  ...
  const handleCalculatePayroll = async () => {
    ...
    const updatedPeriods = payrollPeriods.map(p => p.id === selectedPeriod.id ? updatedPeriod : p)
    setPayrollPeriods(updatedPeriods)
    
    // Actualizar localStorage
    const savedPayrolls = updatedPeriods.filter(p => !mockPayrollPeriods.some(mock => mock.id === p.id))
    localStorage.setItem('payrollPeriods', JSON.stringify(savedPayrolls))
